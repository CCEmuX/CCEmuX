...
object KeyTranslator {
	val swingToCCMap = ImmutableBiMap.copyOf(object : HashMap<Int, Int>() {
		init {
			put(KeyEvent.VK_1, 2);
			put(KeyEvent.VK_2, 3);
			put(KeyEvent.VK_3, 4);
	...
	fun translateToCC(keycode: Int) = swingToCCMap.getOrElse(keycode, { 0 });

	fun translateToSwing(keycode: Int) = swingToCCMap.inverse().getOrElse(keycode, { 0 });
}
